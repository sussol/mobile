...
export class Synchroniser {
  ...
  /**
   * Should only be run after a sync.
   * Processes the local database Requisitions and Transactions of type 'supplier_invoice' that
   * have a serial number -1, and updates them with the appropriate next sequence number. These
   * records have been generated by the server as the other half of actions in other stores:
   * Another site making a requisition to this store, or a supplier invoice from another store
   */
  allocateSerialNumbers() {
    console.log('We are allocating!');
    const requisitions = this.database.objects('Requisition').filtered('serialNumber == "-1"');
    const supplierInvoices = this.database
      .objects('Transaction')
      .filtered('serialNumber == "-1" AND type == "supplier_invoice"');

    if (requisitions.length < 1 && supplierInvoices.length < 1) return;
    this.database.write(() => {
      requisitions.forEach(requisition => {
        console.log('reqbefore', requisition.serialNumber);
        requisition.serialNumber = getNextNumber(this.database, REQUISITION_SERIAL_NUMBER);
        this.database.save('Requisition', requisition);
        console.log('reqafter', requisition.serialNumber);
      });
      supplierInvoices.forEach(supplierInvoice => {
        console.log('supbefore', supplierInvoice.serialNumber);
        supplierInvoice.serialNumber = getNextNumber(this.database, SUPPLIER_INVOICE_NUMBER);
        this.database.save('Transaction', supplierInvoice);
        console.log('supafter', supplierInvoice.serialNumber);
      });
    });
  }
}
