...
export class Synchroniser {
  ...
  async initialise(serverURL, syncSiteName, syncSitePassword, setProgress) {
    ...
    this.allocateSerialNumbers(); // allocateSerialNumbers for any records with serialNumber === -1
  }
  ...
  async synchronise() {
    ...
    await this.push();
    await this.pull();
    await this.allocateSerialNumbers(); // allocateSerialNumbers for any records with serialNumber === -1
   ...
   * Processes the given database for records of serial number -1, and updates them with the
   * appropriate next sequence number. These records have been generated by the server as
   * the other half of actions in other stores: Another site making a requisition here, or
   * a supplier invoice from another store
   * @param  {Realm}  database   The local database
   */
  async allocateSerialNumbers() {
    // TODO: Create data base write, query appropriate tables for serialNumber being -1, assign correct, write finish write
    const sequenceKeys = {
      customer_invoice: CUSTOMER_INVOICE_NUMBER,
      customer_credit: CUSTOMER_INVOICE_NUMBER,
      inventory_adjustment: INVENTORY_ADJUSTMENT_SERIAL_NUMBER,
      requisition: REQUISITION_SERIAL_NUMBER,
      REQUISITION_REQUESTER_REFERENCE,
      STOCKTAKE_SERIAL_NUMBER,
      supplier_invoice: SUPPLIER_INVOICE_NUMBER,
      supplier_credit: SUPPLIER_INVOICE_NUMBER,
    }[type];

    // Handle server generated transactions. If invoice_num is -1 mobile should
    // generate the serialNumber
    const serialNumber =
      record.invoice_num >= 0 ? record.invoice_num : getNextNumber(this.database, sequenceKey);
  }
}
