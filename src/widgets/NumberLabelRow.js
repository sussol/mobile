 ...
 * @prop {Bool}   isCurrency Indicator whether to appends `$` to each number value.
 */
export const NumberLabelRow = ({ size, text, number, total, isCurrency }) => {
  ...
  return (
    <FlexRow flex={1}>
      ...
      <FlexView flex={1}>
        <SimpleLabel text={withTotalText} size={size} textAlign="right" />
      </FlexView>
    </FlexRow>
  );
};
